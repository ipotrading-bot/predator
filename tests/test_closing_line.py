...
def _sig(id_, match, pinnacle_price, match_time):
    return {
        "id": id_,
        "match": match,
        "sport": "soccer",
...
class TestCaptureClosingLines:
    def test_clv_real_computed_from_scan_vs_closing_price(self, monkeypatch):
        now = datetime.now(timezone.utc)
        sig = _sig(1, "A vs B", pinnacle_price=2.00,
                   match_time=(now + timedelta(minutes=3)).isoformat())
        sb = _FakeSupabase([sig])
        monkeypatch.setattr(audit_engine, "get_pinnacle_price",
                            lambda match, sport, league: (1.80, "pinnacle"))
        ...
        assert payload["clv_pct_real"] == round((2.00 / 1.80 - 1) * 100, 2)

    def test_budget_limits_oracle_calls(self, monkeypatch):
        now = datetime.now(timezone.utc)
        sigs = [_sig(i, f"M{i}", 2.0, (now + timedelta(minutes=2)).isoformat()) for i in range(5)]
