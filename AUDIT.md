...
| Les TROIS voies de dépense (closing line imminente, fenêtre favorable, fond) sont adossées au plafond intra-journée : aucune ne peut reprendre l'allocation du jour au premier tick — la closing line garde sa part la plus haute, pas une assiette hors du temps | `tests/test_scan_windows.py::TestRythme::test_closing_line_imminente_reste_prioritaire_mais_suit_l_heure`, `…::test_le_tick_de_nuit_ne_peut_plus_manger_la_soiree` |
...
| `meta.perimetre_ligues_exclues` (motifs de libellé, « ; ») écarte un match AVANT toute requête, libellé replié (accents, phase), chaque refus loggé `HORS PÉRIMÈTRE` ; sans clé, rien ne change | `tests/test_perimetre.py::TestLiguesExcluesParLOperateur` |
| Une LIGUE de jeunes (youth, U17–U23, primavera) est hors périmètre AVANT la garde ESPN, homonyme senior ou non (décision opérateur 2026-09-08) ; au règlement et à la relance des expirés, le nom cherché porte l'étage que la ligue connaît (`nom_avec_etage`), donc le score des seniors ne règle jamais le pari des jeunes | `tests/test_ligues_jeunes.py` |
| Le calculateur `/system` affiche des retours CONDITIONNELS (si k bons sur N : pire cas, meilleur cas, point mort), jamais des probabilités ni un taux de réussite ; le bloc MATH reste exactement ses cinq fonctions ; les scénarios sont EXACTS toutes sections confondues (masques de bits, jamais un rappel de `systemReturn` par scénario) | `tests/test_system_page.py::TestScenariosDerivesDuMoteur`, `::TestSectionsAdditionnees` |
| Sur `/system`, brut = AVANT impôt et net = APRÈS ; la retenue porte sur le gain net de chaque pari GAGNANT (modèle `core/constants.net_b` — un perdant ou un remboursement n'est jamais taxé) et passe par le point unique `taxOfBet` ; le taux vient d'un CHAMP opérateur, jamais d'une constante dans le template, et reste délibérément indépendant de `core.constants.TAX_RATE` qui pilote l'ÉMISSION | `tests/test_system_page.py::TestFiscaliteAppliqueePartout` |
...
| Toute URL ESPN porte UN jour de 8 chiffres (la plage `dates=A-B` rend HTTP 400, lu comme « sport non couvert » : 47 h d'émission fermée le 2026-09-15) ; un sport réglé hors ESPN ne paie pas de fixtures | `tests/test_score_sources.py::TestESPN::test_jamais_une_plage_de_dates`, `tests/test_perimetre.py::TestESPNFixtures::test_les_fixtures_dun_sport_regle_hors_espn_ne_sont_pas_payees` |
| Un marché de COMPTE (total, handicap) ne se règle qu'avec un DÉCOMPTE : pour un sport dont le score est un drapeau de vainqueur (tennis, MMA → 1-0), les jeux d'ESPN (`linescores`) ; sans eux UNKNOWN, jamais un WIN/LOSS — sinon tout « Moins de » gagne et tout « Plus de » perd | `tests/test_totaux_tennis.py` |
| L'ORDRE de `SPORT_KEYS` décide de la dépense quand le plafond refuse une ligue peuplée : rang de FAMILLE (`DEBUTS_DE_FAMILLE`, dérivé de l'ordre déclaré) d'abord, volume seulement DANS la famille ; aucune clé d'une ligue retirée (`LIGUES_RETIREES`) n'y revient, et ses lignes restent réglables | `tests/test_ordre_de_depense.py` |
...
| Titan007 ne redemande pas un fichier de cotes connu absent depuis < `SANS_COTES_TTL_H` | `tests/test_titan007.py::test_un_sid_sans_cotes_recemment_nest_pas_redemande` |
