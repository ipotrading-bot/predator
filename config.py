...
Toutes les valeurs sensibles sont chargées depuis .env
"""
...
class Settings(BaseSettings):
    ...
    # ── PAIM Engine ───────────────────────────────────────────
    min_ev_threshold: float = Field(default=0.08)       # EV+ > 8%
    min_snr_ratio: float = Field(default=1.5)           # Signal-to-Noise
    kelly_fraction: float = Field(default=0.25)         # Fractional Kelly
    max_drawdown_pct: float = Field(default=0.15)       # Kill-switch 15%
    starting_bankroll: float = Field(default=10_000.0)  # Capital initial
    ...
    # ── Rate Limiting ─────────────────────────────────────────
    api_requests_per_minute: int = Field(default=15)    # 15 RPM max
    jitter_min_seconds: float = Field(default=1.5)
    jitter_max_seconds: float = Field(default=4.0)

    # ── Smart Staking ─────────────────────────────────────────
    stake_rounding_base: int = Field(default=10)        # Arrondi à 10€

    # ── Scan Windows ──────────────────────────────────────────
    scan_hours_ahead: int = Field(default=24)
    scan_schedule: list[str] = Field(
        default=["00:00", "08:00", "16:00"]             # 3 scans / jour
    )
