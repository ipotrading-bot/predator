...
# All heavy imports are lazy: numpy, pandas, fpdf, supabase
# (they are only loaded when get_performance_report() is called)
...
class QuantAnalytics:
    ...
    def __init__(self):
        self._supabase_instance = None
        self._np = None
        self._pd = None

    def _init_np_pd(self):
        """Lazy import numpy & pandas."""
        if self._np is None:
            import numpy as np
            self._np = np
        if self._pd is None:
            import pandas as pd
            self._pd = pd
        return self._np, self._pd

    def _get_supabase(self):
        """Lazy initialize Supabase client."""
        if self._supabase_instance is not None:
            return self._supabase_instance
        try:
            ...
            if not url or not key:
                ...
                self._supabase_instance = create_client(url, key)
        except Exception:
            self._supabase_instance = None
        return self._supabase_instance
    ...
    def _calculate_sharpe(self, daily_returns, risk_free_rate: float = 0.02) -> float:
    ...
    def _calculate_sortino(self, daily_returns, risk_free_rate: float = 0.02) -> float:
    ...
    def _calculate_streak(self, outcomes, target: str) -> int:
        """
    ...
    def _calculate_brier_score(self, df) -> float:
        """
...
# Singleton — instantiation won't import supabase/pandas/fpdf
